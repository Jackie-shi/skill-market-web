...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-gray-800 bg-gray-950/80 backdrop-blur-xl">
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="sm:hidden border-t border-gray-800 bg-gray-950 px-4 pb-4 pt-2 space-y-3">
          <form onSubmit={handleSearch}>
            <input
              value={q}
              onChange={(e) => setQ(e.target.value)}
              placeholder="Search skills..."
              className="w-full rounded-lg bg-gray-900 border border-gray-700 px-4 py-2 text-sm text-white placeholder-gray-500 focus:border-emerald-500 focus:outline-none"
            />
          </form>
          <Link href="/search" className="block text-sm text-gray-400 hover:text-white" onClick={() => setMobileOpen(false)}>Browse All</Link>
          <Link href="/publish" className="block rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white text-center" onClick={() => setMobileOpen(false)}>Publish Skill</Link>
          {session ? (
            <>
              <Link href="/profile" className="block text-sm text-gray-400 hover:text-white" onClick={() => setMobileOpen(false)}>My Profile</Link>
              <button onClick={() => signOut()} className="block text-sm text-red-400 hover:text-red-300">Sign Out</button>
            </>
          ) : (
            <button onClick={() => signIn("github")} className="block text-sm text-gray-400 hover:text-white">Sign In with GitHub</button>
          )}
        </div>
      )}
    </header>
