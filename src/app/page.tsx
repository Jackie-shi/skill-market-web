    ...
    <div className="space-y-16 pb-16">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pt-20 pb-16 sm:px-6 text-center">
          ...
          {!loading && skills.length > 0 && (
            <div className="mt-6 flex items-center justify-center gap-6 text-sm text-gray-500">
              <div className="flex items-center gap-1.5">
                <span className="text-emerald-400 font-semibold">{skills.length}</span> skills available
              </div>
              <div className="w-px h-4 bg-gray-700" />
              <div className="flex items-center gap-1.5">
                <span className="text-emerald-400 font-semibold">
                  {skills.reduce((sum, s) => sum + (s.downloads ?? 0), 0).toLocaleString()}
                </span> total installs
              </div>
              <div className="w-px h-4 bg-gray-700 hidden sm:block" />
              <div className="hidden sm:flex items-center gap-1.5">
                <span className="text-emerald-400 font-semibold">
                  {new Set(skills.flatMap(s => s.compatibility?.platforms ?? [])).size}
                </span> platforms supported
              </div>
            </div>
          )}
            ...
            <div className="mt-8 space-y-4">
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-lg bg-white text-gray-900 px-6 py-3 text-sm font-semibold hover:bg-gray-100 transition-colors shadow-lg shadow-white/10"
                >
                ...
                <Link href="/search" className="text-sm text-gray-400 hover:text-gray-300 transition-colors">
                  or browse skills first →
                </Link>
              </div>
              <div className="flex items-center justify-center gap-4 text-xs text-gray-600">
                <span className="flex items-center gap-1">✓ Free to browse &amp; install</span>
                <span className="flex items-center gap-1">✓ Publish in 5 min</span>
                <span className="flex items-center gap-1">✓ Earn from day one</span>
              </div>
            </div>
          )}
          ...
          {session && (
            <div className="mt-8 space-y-4">
              {/* Welcome banner for users with no skills */}
              {!loading && skills.length > 0 && (
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                <Link href="/dashboard" className="inline-flex items-center gap-2 rounded-lg border border-gray-700 px-4 py-2 text-sm text-gray-300 hover:border-emerald-500/50 hover:text-white transition-all">
                  📊 Dashboard
                </Link>
                <Link href="/publish" className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-500 transition-colors">
                  ✨ Publish a Skill
                </Link>
                <Link href="/profile" className="inline-flex items-center gap-2 rounded-lg border border-gray-700 px-4 py-2 text-sm text-gray-300 hover:border-emerald-500/50 hover:text-white transition-all">
                  👤 My Profile
                </Link>
              </div>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-950/50 to-gray-950 p-8 sm:p-12">
          <div className="text-center mb-8">
            <Tooltip content="Join now — early creators keep 100% revenue for 6 months" position="top">
              <div className="inline-flex items-center gap-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 px-4 py-1.5 text-sm text-emerald-400 mb-4 cursor-help">
